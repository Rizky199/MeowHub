...
-- MakeDraggable khusus CatBtn: bedakan drag vs tap
-- isDragging diexport agar DoRestore bisa cek
local catIsDragging = false
local function MakeCatDraggable(handle, target)
    local drag    = false
    local moved   = false
    local ds, sp  = nil, nil
    local THRESHOLD = 6  -- pixel minimum untuk dianggap drag
    ...
    UserInputService.InputEnded:Connect(function(i)
        ...
        or i.UserInputType == Enum.UserInputType.Touch then
            drag = false
            -- Reset flag setelah delay kecil (setelah MouseButton1Click fired)
            task.delay(0.05, function() catIsDragging = false end)
        end
    end)
end
...
MakeCatDraggable(CatBtn, CatBtn)
...
local function DoRestore()
    ...
    -- Hitung posisi GUI agar tidak keluar layar
    local screenW = workspace.CurrentCamera.ViewportSize.X
    local screenH = workspace.CurrentCamera.ViewportSize.Y
    local gx = cx - WIN_W / 2 + 26
    local gy = cy
    -- Clamp agar tidak keluar layar
    gx = math.clamp(gx, 0, screenW - WIN_W)
    gy = math.clamp(gy, 0, screenH - WIN_H)
    ...
    task.delay(0.18, function()
        CatBtn.Visible = false
        Main.Position = UDim2.new(0, gx, 0, gy)
        Main.BackgroundTransparency = 1
        Main.Visible = true
...
CatBtn.MouseButton1Click:Connect(function()
    if catIsDragging then return end  -- abaikan jika ini hasil drag
    DoRestore()
end)
